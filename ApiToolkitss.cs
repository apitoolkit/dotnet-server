...
namespace ApiToolkiiiiiiiiiiiiit.Net
{
    ...
    public class ObservingHandler : DelegatingHandler
    {
        ...
        public ObservingHandler(Func<Payload, Task> publishMessage, string project_id, HttpContext? httpContext = null, ATOptions? options = null) : base(new HttpClientHandler())
        {
            ...
            _options = options ?? new ATOptions { RedactHeaders = [], RedactRequestBody = [], RedactResponseBody = [] };
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var payload = new Payload
                {
                    ...
                    Referer = "",
                    RequestBody = Client.RedactJSON(System.Text.Encoding.UTF8.GetBytes(reqBody), _options.RedactRequestBody ?? []),
                    RequestHeaders = Client.RedactHeaders(reqHeaders, _options.RedactHeaders ?? []),
                    ResponseBody = Client.RedactJSON(System.Text.Encoding.UTF8.GetBytes(respBody), _options.RedactResponseBody ?? []),
                    ResponseHeaders = Client.RedactHeaders(respHeaders, _options.RedactHeaders ?? []),
